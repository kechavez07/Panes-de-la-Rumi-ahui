...
app.get('/product_promotion/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('SELECT * FROM product_promotion WHERE id_promotion = $1 OR id_product = $1', [id]);
    res.json(result.rows);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Error');
...
app.delete('/product_promotion/:id', async (req, res) => {
  try {
    const { id } = req.params;
    await pool.query('DELETE FROM product_promotion WHERE id_promotion = $1 OR id_product = $1', [id]);
    res.status(204).send();
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Error');
...
// Additional blog and category routes are placed below the original sections.
